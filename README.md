...
4. 結果パネルでテストを選ぶと、右側に入力・期待出力・実際の出力が表示される
5. `e` で選択中の問題のソースを `$EDITOR` で開く
6. `L` でログインし、`s` で提出 (確認ダイアログあり)
...
コンテスト一覧は [platformdirs](https://github.com/tox-dev/platformdirs) が示すユーザ設定ディレクトリ配下に保存されます。ログインセッション (cookie)、提出言語、最後に開いたコンテスト/問題は `$XDG_STATE_HOME/atcoder-tui` に保存されます。問題文 Markdown は `$XDG_CACHE_HOME/atcoder-tui` に保存されます。未設定時はそれぞれ XDG 標準の `~/.local/state/atcoder-tui`、`~/.cache/atcoder-tui` を使用します。
